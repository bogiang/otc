...
class TradeModel extends \Think\Model
{
	...
	public function chexiao($id = NULL)
	{
		try{
			if (!check($id, 'd')) {
				throw new \Think\Exception('参数错误');
			}

			$trade = M('Trade')->where(array('id' => $id))->find();

			if (!$trade) {
				throw new \Think\Exception('订单不存在');
			}

			if ($trade['status'] != 0) {
				throw new \Think\Exception('此订单不能撤销');
			}
			...
			if (!$xnb) {
				throw new \Think\Exception('卖出市场错误');
			}
			
			if (!$rmb) {
				throw new \Think\Exception('买入市场错误');
			}
			...
			if ($fee_buy < 0) {
				throw new \Think\Exception('买入手续费错误');
			}
			if ($fee_sell < 0) {
				throw new \Think\Exception('卖出手续费错误');
			}
			...
			if ($trade['type'] == 1) {
				...
				if($save_buy_rmb<=0){
					throw new \Think\Exception('撤销失败，返还金额0');
				}
				...
				$rs[] = $mo->table('tw_finance_log')->add(array('username' => $uu_name, 'adminname' => '系统', 'addtime' => time(), 'plusminus' => 0, 'amount' => $save_buy_rmb, 'optype' => 25, 'cointype' => 1, 'old_amount' => $user_buy[$rmb . 'd'], 'new_amount' => $user_buy[$rmb . 'd']-$save_buy_rmb, 'userid' => $uu_id,'addip'=>get_client_ip(),'position'=>$position));
				// 处理资金变更日志-----------------E
				$finance_hash = md5($trade['userid'] . $finance_num_user_coin['cny'] . $finance_num_user_coin['cnyd'] . $save_buy_rmb . $finance_mum_user_coin['cny'] . $finance_mum_user_coin['cnyd'] . MSCODE . 'tp3.net.cn');
				...
				if (0 < $save_sell_xnb) {
					...
					// 处理资金变更日志-----------------E
					// optype动作类型 'cointype' => 资金类型 'plusminus' => 1增加类型
					$rs[] = $mo->table('tw_finance_log')->add(array('username' => $uu_name, 'adminname' => '系统', 'addtime' => time(), 'plusminus' => 0, 'amount' => $user_sell[$xnb . 'd'], 'optype' => 26, 'cointype' => $coin_info['id'], 'old_amount' => $user_sell[$xnb . 'd'], 'new_amount' => $user_sell[$xnb . 'd']-$save_sell_xnb, 'userid' => $uu_id,'addip'=>get_client_ip(),'position'=>$position));
					// 处理资金变更日志-----------------E
				}else{
					throw new \Think\Exception('撤销失败，返还金额0');
				}
			...
			$reason = $e->getMessage();
			if($reason=='参数错误'||$reason=='订单不存在'||$reason=='此订单不能撤销'||$reason=='卖出市场错误'||$reason=='买入市场错误'||$reason=='买入手续费错误'||$reason=='卖出手续费错误'){
				return array('0', $reason);
			}elseif($reason=='撤销失败3'){
				$mo->execute('rollback');
				$mo->execute('unlock tables');
				return array('0', '撤销失败3');
			}elseif($reason=='撤销失败，返还金额0'){
				$mo->execute('rollback');
				$mo->execute('unlock tables');
				return array('0', '撤销失败，返还金额0');
			}else{
				$mo->execute('rollback');
				$mo->execute('unlock tables');
				return array('0', $reason);
