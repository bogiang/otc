...
class UserController extends AdminController
{
	protected function _initialize(){
		parent::_initialize();
		$allow_action=array("index","edit","status","admin","adminEdit","adminStatus","auth","authEdit","authStatus","authStart","authAccess","updateRules","authAccessUp","authUser","authUserAdd","authUserRemove","log","logEdit","logStatus","qianbao","qianbaoEdit","qianbaoStatus","coin","coinEdit","coinLog","setpwd","userExcel","nameauth","nameauthdetail","invittree","financelog");
		if(!in_array(ACTION_NAME,$allow_action)){
			$this->error("页面不存在！");
		}
	}
	
	public function region($name = NULL, $field = NULL)
	{
		$where = array();
		...
		$show = $Page->show();
		$list = M('Region')->where($where)->order('id desc')->limit($Page->firstRow . ',' . $Page->listRows)->select();
		
		foreach($list as $k=>$v){
			$list[$k]['dluser'] = M('User')->where(array('id'=>$v['dluser']))->getField('username');
		}
	...
	public function regionEdit($id = NULL)
	{
		if (empty($_POST)) {
			if (empty($id)) {
				$this->data = null;
				...
				$this->data = M('Region')->where(array('id' => trim($id)))->find();
			}
			...
			$_POST['addtime'] = time();
			if(!empty($_POST['dluser'])){
				$_POST['dluser'] = M('User')->where(array('username'=>$_POST['dluser']))->getField('id');
			}
			if(empty($_POST['id'])){
				$res = M('Region')->add($_POST);
			}else{
				$res = M('Region')->save($_POST);
			}

			if ($res) {
				if(!empty($_POST['dluser'])){
					$record = M('Region')->where(array('dluser'=>$_POST['dluser']))->find();
					if(!empty($record)){
						M('User')->where(array('id'=>$record['dluser']))->save(array('region'=>$record['id']));
					}
				}
				$this->success('编辑成功！');
			}
	...
	public function regionDelete($id = NULL){
		if(!empty($id)){
			$region_user = M('User')->where(array('region'=>array('in',$id)))->find();
			if(!empty($region_user)){
				$this->error('该地区下还有用户，不能删除！');
			}
			$res = M('Region')->where(array('id'=>array('in',$id)))->delete();
			if($res){
				$this->success('删除成功！');
			}else{
				$this->success('删除失败！');
	...
	public function invittree($userid = NULL){
		if(empty($userid)){
			...
			if ($field && $name) {
				...
				$where['invit_1'] = 0;
				$where['id'] = array('in',$leaderids);
			}
			$count = M('User')->where($where)->count();
			$Page = new \Think\Page($count, 15);
			$show = $Page->show();
			$userlist = M('User')->where($where)->field('id,username,truename,mobile,addtime')->order('id asc')->limit($Page->firstRow . ',' . $Page->listRows)->select();
	...
	public function tradePrize($name=NULL,$field=NULL,$starttime=NULL,$endtime=NULL){
		$where=array();
		if ($field && $name) {
			$where[$field] = $name;
		}
		...
		$count = M('trade_prize')->where($where)->count();
		$Page = new \Think\Page($count, 15);
		$show = $Page->show();
		$list = M('trade_prize')->where($where)->order('id desc')->limit($Page->firstRow . ',' . $Page->listRows)->select();
		$this->assign('list', $list);
		$this->assign('page', $show);
		$this->display();
	}
	...
	public function nameauthdetail(){
		if (empty($_POST)) {
			...
			if($pass==2){
				$rs = M('User')->where(array('id' => $uid))->save(array('is_agree'=>2,'idcard_zheng'=>'','idcard_fan'=>'','idcard_shouchi'=>'','freason'=>$_POST['freason']));
				if($rs){
					$this->success("审核成功！","/Admin/User/nameauth.html");
				}else{
					$this->error('审核失败！');
	...
	public function index($name = NULL, $field = NULL, $status = NULL)
	{
		...
		foreach ($list as $k => $v) {
			...
			$list[$k]['region'] = M('Region')->where(array('id'=>$v['region']))->getField('rgname');
		}
	...
	public function edit($id = NULL)
	{
		if (empty($_POST)) {
			if (empty($id)) {
				...
				$result = M('User')->where(array('id' => trim($id)))->find();
				$this->data = $result;
				$usercoin = M('UserCoin')->where(array('userid' => trim($id)))->find();
			...
			if(!empty($_POST['idcard']) && !validation_filter_id_card($_POST['idcard'])){
				// $this->error('证件号码格式错误！');
			}
			...
			if($_FILES['file']['name']) {
				...
				if ($info) {
					...
					$headroot = '/Public/Home/images/hportrait/' . $file['savepath'] . "s_" . $file['savename'];
					$_POST['headimg'] = $headroot;

					if ($id == null) {
						$_POST['mobiletime'] = time();
						$_POST['addtime'] = time();
						$mo = M();
						$mo->startTrans();
						$rs = array();
						...
						$headimg = M('User')->where(array('id' => $id))->getField('headimg');
						if ($headimg != '') {
							unlink('.' . $headimg);
						}
	...
	public function adminEdit()
	{
		if (empty($_POST)) {
			...
			if ($input['username']!='admin' && !check($input['username'], 'username') && !check($input['username'],'mobile') && !check($input['username'],'email')) {
				$this->error('用户名格式错误！');
			}
	...
	public function coin($name = NULL, $field = NULL)
	{
		...
		$coin_list = M('Coin')->where("name='usdt' or name='eth'")->select();
	...
	// 资金变更日志
	public function financelog($position = 'all', $plusminus = 'all', $name = NULL, $field = NULL, $cointype = NULL, $optype = NULL, $starttime = NULL, $endtime = NULL)
	{
		$where = array();
		...
		foreach ($list as $k => $v) {
			...
			$list[$k]['old_amount'] = numform($v['old_amount']);
			$list[$k]['amount'] = numform($v['amount']);
			$list[$k]['new_amount'] = numform($v['new_amount']);
			if($v['plusminus']){
				$list[$k]['plusminus'] = '增加';
			}else{
				$list[$k]['plusminus'] = '减少';
